...
$(document).ready(function(){$(".tap-map").height($(window).height()-80),$("#map").length&&(map=new GMaps({div:"#map",lat:51.5005,lng:-2.548,zoom:17}),map.addMarker({lat:51.501629,lng:-2.548818,icon:"/build/img/map-marker-tappoint.png",title:"Cribs Q",infoWindow:{content:"<p>Cribs Q</p><p>358 tap-ins this month</p>"}}),map.addMarker({lat:51.501285,lng:-2.548807,icon:"/build/img/map-marker-tappoint.png",title:"Q Block Info Point",infoWindow:{content:"<p>Q Block Info Point</p><p>129 tap-ins this month</p>"}}),map.addMarker({lat:51.50112,lng:-2.548608,icon:"/build/img/map-marker-tappoint.png",title:"Q Block Pond",infoWindow:{content:"<p>Q Block Pond</p><p>980 tap-ins this month</p>"}}),map.addMarker({lat:51.500395,lng:-2.548038,icon:"/build/img/map-marker-tappoint.png",title:"Full Of Beans",infoWindow:{content:"<p>Full Of Beans</p><p>84 tap-ins this month</p>"}}),map.addMarker({lat:51.500295,lng:-2.547956,icon:"/build/img/map-marker-tappoint.png",title:"Jimmy Deane's Fruit & Veg",infoWindow:{content:"<p>Jimmy Deane's Fruit &amp; Veg</p><p>102 tap-ins this month</p>"}}),map.addMarker({lat:51.5002,lng:-2.547376,icon:"/build/img/map-marker-tappoint.png",title:"The SU Shop",infoWindow:{content:"<p>The SU Shop</p><p>577 tap-ins this month</p>"}}),map.addMarker({lat:51.500061,lng:-2.547478,icon:"/build/img/map-marker-tappoint.png",title:"Escape Bar",infoWindow:{content:"<p>Escape Bar</p><p>1036 tap-ins this month</p>"}}),map.addMarker({lat:51.500049,lng:-2.547588,icon:"/build/img/map-marker-tappoint.png",title:"Red Bar",infoWindow:{content:"<p>Red Bar</p><p>712 tap-ins this month</p>"}}),GMaps.geolocate({success:function(n){map.addMarker({lat:n.coords.latitude,lng:n.coords.longitude,icon:"/build/img/map-marker-geolocation.png",title:"You are here.",infoWindow:{content:"<p>You are here!</p>"}})},error:function(n){console.log("Geolocation failed: "+n.message)},not_supported:function(){console.log("Your browser has no geolocation")}}),(markerName=location.hash.substr(1))&&(markerNames=$.map(map.markers,function(n){return n.title.toLowerCase().replace(/\W/g,"")}),index=markerNames.indexOf(markerName),(marker=map.markers[index])&&(map.setCenter(marker.position.lat(),marker.position.lng()),map.setZoom(18))))});
...
$(document).ready(function(){if($("#tapInToBegin, #tapInExplanation").on("click",function(){$("#tapInExplanation").toggleClass("explanation-hidden")}),$(".action-overflow-hidden").removeClass("action-overflow-hidden"),$("#file-button").on("click",function(){$("input[type=file]")[0].click()}),query=$.parseQuery(),"photo"===query.upload){new fries.Toast({content:"Thanks! Your photo will be uploaded shortly."})}else if("text"===query.upload){new fries.Toast({content:"Thanks! Your post will be uploaded shortly."})}});