...
$(document).ready(function(){var e=null;if($("#reportDialog").length)var t=new fries.Dialog({selector:"#reportDialog",callbackOk:function(){e.remove(),this.hide()},callbackCancel:function(){this.hide()}});else var t=!1;$(".card__overlay__section--report").on("click",function(){e=$(this).closest(".card"),t.show()}),$(".card").on("click",function(){$(this).find(".card__overlay").toggleClass("card__overlay--hidden")}).find(".card__overlay__content").on("click",function(e){e.stopPropagation()}),$(".card__overlay__section--like, .card__overlay__section--dislike").on("click",function(){if(verb=$(this).hasClass("card__overlay__section--like")?"Like":"Dislike",rated=$(this).hasClass("js-rated"),$(this).toggleClass("js-rated"),$(this).toggleClass("card__overlay__social-stat--active"),stat=$(this).find(".card__overlay__social-stat"),rating=parseInt(stat.text()),rated){rating--;{new fries.Toast({content:"Removed "+verb+" from this!"})}}else{rating++;{new fries.Toast({content:verb+"d this!."})}}stat.text(rating),$(this).closest(".card__overlay").delay(250).queue(function(){$(this).addClass("card__overlay--hidden"),$(this).dequeue()})})});
!function(e){e.fn.shuffle=function(){var t=this.get(),n=function(e){return Math.floor(Math.random()*e)},c=e.map(t,function(){var c=n(t.length),o=e(t[c]).clone(!0)[0];return t.splice(c,1),o});return this.each(function(t){e(this).replaceWith(e(c[t]))}),e(c)}}(jQuery),$(".filter-menu input[type=radio]").on("change",function(){setTimeout(function(){$(".content .component").shuffle(),$(".filter-menu").hide()},200)}),$(".filter-menu input[type=checkbox]").on("change",function(e){$checkbox=$(e.target),type=$checkbox.attr("id").split("-")[1],checked=$checkbox.is(":checked"),$matchedContent=$(".content .card--"+type),checked?$matchedContent.show():$matchedContent.hide()});
$(document).ready(function(){$("#map").length&&(map=new GMaps({div:"#map",lat:51.5005,lng:-2.548,zoom:17}),map.addMarker({lat:51.501629,lng:-2.548818,icon:"/build/img/map-marker-tappoint.png",title:"Cribs Q",infoWindow:{content:"<p>Cribs Q</p><p>358 tap-ins this month</p>"}}),map.addMarker({lat:51.501285,lng:-2.548807,icon:"/build/img/map-marker-tappoint.png",title:"Q Block Info Point",infoWindow:{content:"<p>Q Block Info Point</p><p>129 tap-ins this month</p>"}}),map.addMarker({lat:51.50112,lng:-2.548608,icon:"/build/img/map-marker-tappoint.png",title:"Q Block Pond",infoWindow:{content:"<p>Q Block Pond</p><p>980 tap-ins this month</p>"}}),map.addMarker({lat:51.500395,lng:-2.548038,icon:"/build/img/map-marker-tappoint.png",title:"Full Of Beans",infoWindow:{content:"<p>Full Of Beans</p><p>84 tap-ins this month</p>"}}),map.addMarker({lat:51.500395,lng:-2.548038,icon:"/build/img/map-marker-tappoint.png",title:"Jimmy Deane's Fruit & Veg",infoWindow:{content:"<p>Jimmy Deane's Fruit &amp; Veg</p><p>102 tap-ins this month</p>"}}),map.addMarker({lat:51.5002,lng:-2.547376,icon:"/build/img/map-marker-tappoint.png",title:"The SU Shop",infoWindow:{content:"<p>The SU Shop</p><p>577 tap-ins this month</p>"}}),map.addMarker({lat:51.500061,lng:-2.547478,icon:"/build/img/map-marker-tappoint.png",title:"Escape Bar",infoWindow:{content:"<p>Escape Bar</p><p>1036 tap-ins this month</p>"}}),map.addMarker({lat:51.500049,lng:-2.547588,icon:"/build/img/map-marker-tappoint.png",title:"Red Bar",infoWindow:{content:"<p>Red Bar</p><p>712 tap-ins this month</p>"}}),GMaps.geolocate({success:function(n){map.setCenter(n.coords.latitude,n.coords.longitude),map.addMarker({lat:n.coords.latitude,lng:n.coords.longitude,icon:"/build/img/map-marker-geolocation.png",title:"You are here.",infoWindow:{content:"<p>You are here!</p>"}})},error:function(n){console.log("Geolocation failed: "+n.message)},not_supported:function(){console.log("Your browser has no geolocation")}}))});
$(function(){hammertime=Hammer(document.body,{swipeVelocityX:.2}).on("dragright, swiperight",function(){$(".main-navigation").addClass("main-navigation--open")}),hammertime=Hammer(document.body,{swipeVelocityX:.2}).on("dragleft, swipeleft",function(){$(".main-navigation").removeClass("main-navigation--open")}),Hammer($("#menu-toggle")[0]).on("tap",function(){$(".main-navigation").toggleClass("main-navigation--open")}),$(".filter-menu").hide(),$(".toggle-filter-menu").on("click",function(){$(".filter-menu").toggle()})});
$(document).ready(function(){$("#tapInToBegin, #tapInExplanation").on("click",function(){$("#tapInExplanation").toggleClass("explanation-hidden")})});